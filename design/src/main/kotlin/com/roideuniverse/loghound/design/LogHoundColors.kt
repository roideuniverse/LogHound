 ...
 *
 * The four predefined instances live in [LogHoundThemes]. Composables read the active theme via
 * [LocalLogHoundColors].current rather than reaching into [LogHoundDesign.Colors] directly — the
 * latter stays as a Light-theme alias for non-composable callers (DSL plugins, unit tests, etc.).
 *
 * Token naming follows the Claude Design prototype (2026-06-28). The mapping from prototype CSS
 * variables to Kotlin names is documented in specs/design/swiss-direction.md → Claude Design
 * Revision.
 */
...
fun LogHoundColors.colorFor(priority: LogPriority): Color =
    when (priority) {
...
fun LogHoundColors.badgeBgFor(priority: LogPriority): Color =
    when (priority) {
...
/**
 * Active theme injected by the window shell via
