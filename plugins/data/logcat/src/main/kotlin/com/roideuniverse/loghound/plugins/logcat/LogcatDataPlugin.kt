 ...
 *
 * The plugin owns a small supervisor coroutine that polls `adb devices` every [POLL_INTERVAL_MS],
 * diffs the result against its in-memory map of `serial -> stream Job`, and reconciles:
 * - serials that newly appeared get a fresh `streamFromDevice` coroutine
 * - serials that disappeared have their stream cancelled (which destroys the subprocess, unblocking
 *   the readLine() loop)
 * - serials that are still present are left alone
 *
 * The detected set is also published to [LogRepository.devices] so the UI status-bar pill /
 * autocomplete / per-device tabs can render without having to shell out themselves.
 *
 * Each per-device stream tags its emitted `LogEntry`s with `DeviceId(serial)` so downstream
 * consumers (filter, UUID Grouping, sessions) can scope by source.
 */
class LogcatDataPlugin @Inject constructor() : DataPlugin {
    ...
    private suspend fun streamFromDevice(adb: String, serial: String, repository: LogRepository) =
        coroutineScope {
    ...
    private suspend fun listAttachedDevices(adb: String): Set<String> =
        withContext(Dispatchers.IO) {
