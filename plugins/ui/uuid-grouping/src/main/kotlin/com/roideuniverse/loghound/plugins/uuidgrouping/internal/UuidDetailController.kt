 ...
 *
 * Owns the matching log entries, a `LazyListState` for scroll position retention across tab
 * switches, observable flags for the UI's loading and load-older states, and the coroutine `Job`
 * that streams matching entries into the list.
 *
 * Lookup uses the plugin's per-occurrence `uuid_log` table for an indexed `SELECT log_id WHERE uuid
 * = ?`, then fetches those entries from the main logs DB by id. This is constant-time per UUID; the
 * previous implementation scanned the main logs DB page-by-page until it collected enough matches,
 * which scaled with total log volume rather than per-UUID occurrence count (and was pathological
 * for low-count UUIDs that never reach the limit).
 *
 * `start` is idempotent — safe to call once when the tab is opened. `stop` cancels the collection
 * cleanly when the tab is closed.
 */
internal class UuidDetailController(
    ...
    /**
     * True from construction until the initial query completes (or fails). Observable Compose state
     * — drives the loading-vs-empty-vs-loaded rendering.
     */
    ...
    /**
     * True once a load-older fetch has returned zero entries. The trigger then stops firing for the
    ...
    /**
     * Fetch the next page of older entries for this UUID and prepend to [entries]. Reentry guarded
     * — concurrent calls collapse into a single fetch. Idempotent once exhausted.
     */
    ...
    private suspend fun fetchInitial(): List<LogEntry> {
        val ids =
            withContext(Dispatchers.IO) {
                uuidQueries
                    .selectLogIdsForUuidDesc(uuid = uuid, lim = INITIAL_LIMIT.toLong())
                    .executeAsList()
            }
